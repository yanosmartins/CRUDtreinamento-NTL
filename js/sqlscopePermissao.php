...
function grava(){
    ...
    $sql = "usuarioFuncionalidade_AtualizaPermissoesUsuario (".$idUsuario.",".$usuario.",".$xmlFuncionalidade.") ";    
    $reposit = new reposit();  
...
function recupera(){
    ...
    $sql=" SELECT USUF.codigo,USUF.funcionalidade
           FROM dbo.usuarioFuncionalidade USUF
           INNER JOIN dbo.usuario USU on USU.codigo = USUF.usuario
    ...
    $sql=" SELECT FNC.codigo,FNC.nome,FNC.nomeCompleto,FNC.menuItem
           FROM dbo.funcionalidade FNC
           INNER JOIN dbo.menuItem SMI ON SMI.codigo = FNC.menuItem ";
    $where = "WHERE (0=0) ";
    ...
    while(($row=odbc_fetch_array($result)))
    {
        ...
        $arrayFuncionalidade[] = array("idFuncionalidade" =>$idFuncionalidade,
                                       "nome" =>$nome,
