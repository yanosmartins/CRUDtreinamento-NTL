    ...
    <div class="table-responsive" style="min-height: 115px; border: 1px solid #ddd; margin-bottom: 13px; overflow-x: auto;">
        <table id="tableSearchResult" class="table table-bordered table-striped table-condensed table-hover dataTable">
            ...
            <tbody>
                ...
                PM.solicitante, FS.nome AS descricaoSolicitante, PM.responsavel, FR.nome AS descricaoResponsavel, PM.aprovado, PM.dataCadastramento, PM.tipo,
                    SUBSTRING(
                            (
                                ...
                                WHERE EM.pedidoMaterial = PM.codigo
                                FOR XML PATH ('')
                            ), 2, 1000) [material],
                ...
                if ($_POST["devolvido"] != "") {
                    $devolvido = (int)$_POST["devolvido"];
                    if ($devolvido == 0) {
                        $where = $where . " AND PM.aprovado = 1 AND EXISTS (SELECT TOP 1 pedidoMaterial FROM Estoque.estoqueMovimento WHERE situacaoItem = 4 AND pedidoMaterial = PM.codigo)";
                    }
                    if ($devolvido == 1) {
                        $where = $where . " AND PM.aprovado = 1 AND NOT EXISTS (SELECT TOP 1 pedidoMaterial FROM Estoque.estoqueMovimento WHERE situacaoItem = 4 AND pedidoMaterial = PM.codigo)";
                    }
                } else {
                    $where = $where . "  AND PM.aprovado = 1 AND EXISTS (SELECT TOP 1 pedidoMaterial FROM Estoque.estoqueMovimento WHERE situacaoItem = 4 AND pedidoMaterial = PM.codigo)";
                }
                ...
                foreach ($result as $row) {
                    ...
                    echo '<td class="text-left"><a href="estoque_devolucaoMaterialCadastro.php?id=' . $id . '">' . $descricaoData . '</td>';
