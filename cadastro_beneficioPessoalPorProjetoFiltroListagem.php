        ...
        <table id="tableSearchResult" class="table table-bordered table-striped table-condensed table-hover dataTable">
            <thead>
                <tr role="row">
                    ...
                    <th class="text-left" style="min-width:35px;">Sindicato</th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                $sql = "SELECT BP.projeto, BP.codigo, BP.funcionario, BP.sindicato, P.descricao, F.nome, S.descricao AS descricaoSindicato 
                        FROM Ntl.beneficioProjeto BP
                        LEFT JOIN Ntl.projeto P ON BP.projeto = P.codigo
                        LEFT JOIN Ntl.funcionario F ON BP.funcionario = F.codigo
                        LEFT JOIN Ntl.sindicato S ON BP.sindicato = S.codigo WHERE F.dataDemissaoFuncionario IS NULL";

                $ativo = $_GET["ativo"];
                if ($_GET["ativo"] == 0) {
                    $ativo = $_GET["ativo"];
                    $where =  " AND BP.ativo = 0";
                }else{
                    $ativo = $_GET["ativo"];
                    $where =  " AND BP.ativo = 1";
                }

                $sindicato = $_GET['sindicato'];
                if ($_GET["sindicato"] != 0) {
                    $funcionario = $_GET["sindicato"];
                    $and =  " AND BP.sindicato = " . $sindicato;
                }

                $projeto = $_GET["projeto"];
                if ($_GET["projeto"] != 0) {
                    $projeto = $_GET["projeto"];
                    $and =  " AND projeto = " . $projeto;
                }

                $funcionario = $_GET["funcionario"];
                if ($_GET["funcionario"] != 0) {
                    $funcionario = $_GET["funcionario"];
                    $and =  " AND funcionario = " . $funcionario;
                }
                ...
                while (($row = odbc_fetch_array($result))) {
                    ...
                    echo '<td class="text-left">' . $sindicato . '</td>';
