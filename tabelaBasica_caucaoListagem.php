...
<div class="table-container">
    <div class="table-responsive" style="min-height: 115px; border: 1px solid #ddd; margin-bottom: 13px; overflow-x: auto;">
        <table id="tableSearchResult" class="table table-bordered table-striped table-condensed table-hover dataTable">
            <thead>
                <tr role="row">
                    <th class="text-left" style="min-width:30px;">Tipo de Caução</th>
            ...
            <tbody>
                ...
                while (($row = odbc_fetch_array($result))) {
                    $codigo = +$row['codigo'];
                    $tipoCaucao =  mb_convert_encoding($row['descricao'], 'UTF-8', 'HTML-ENTITIES');
                    $ativo = $row['ativo'];

                    echo '<tr>';
                    echo '<td class="text-left"><a href="tabelaBasica_caucaoCadastro.php?codigo=' . $codigo . '">'  . $tipoCaucao . '</a></td>';
                    if ($ativo == 1) {
